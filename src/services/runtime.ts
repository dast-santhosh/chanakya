...
const DEFAULT_WEB_API_URL = 'https://api.ajnav.com';
...
function isWorldMonitorWebHost(hostname: string): boolean {
  return hostname === 'ajnav.com'
    || hostname === 'www.ajnav.com'
    || hostname.endsWith('.ajnav.com');
}
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://ajnav.com';
  return '';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.ajnav.com');
  } catch {
    return false;
  }
}
...
const ALLOWED_REDIRECT_HOSTS = /^https:\/\/([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)*ajnav\.com(:\d+)?$/;
