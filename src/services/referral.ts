...
export async function shareReferral(profile: ReferralProfile): Promise<ShareResult> {
  ...
  const text = 'Get geopolitical intelligence in a daily editorial brief. Join me on Chanakya Dashboard:';
  // Web Share — mobile primary path.
  if (typeof navigator !== 'undefined' && typeof navigator.share === 'function') {
    try {
      await navigator.share({ title: 'Chanakya Dashboard', text, url });
      return 'shared';
    } catch (err) {
