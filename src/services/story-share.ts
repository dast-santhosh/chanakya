...
export const shareTexts = {
  twitter: (data: StoryData) =>
    `[${data.countryCode}] ${data.countryName} Intelligence Brief\n\n` +
    `Instability: ${data.cii?.score || 'N/A'}/100 (${data.cii?.level || 'N/A'})\n` +
    `${data.threats.critical > 0 ? `[Alert] ${data.threats.critical} critical threats\n` : ''}` +
    `\nGenerated by @AjnavLabs`,
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via Chanakya Dashboard - Open source geopolitical intelligence`,

  telegram: (data: StoryData) =>
    `[${data.countryCode}] *${data.countryName} Intelligence*\n\n` +
    `Instability: *${data.cii?.score || 'N/A'}/100* (${data.cii?.level || 'N/A'})\n` +
    `Trend: *${data.cii?.trend || 'stable'}*\n` +
    `${data.cii?.change24h ? `24h: *${data.cii.change24h > 0 ? '+' : ''}${data.cii.change24h}*\n` : ''}` +
    `${data.threats.critical > 0 ? `Critical: *${data.threats.critical}*\n` : ''}` +
    `${data.threats.high > 0 ? `High: *${data.threats.high}*\n` : ''}` +
    `\nLink: ${generateStoryDeepLink(data.countryCode, 'ciianalysis', data.cii?.score, data.cii?.level)}`
};
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief - Chanakya Dashboard`)}`,
